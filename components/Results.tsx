  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 p-4 md:p-8 animate-fade-in">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 mt-8">
        ...
        <div className="space-y-8">
          <div className="relative w-full aspect-square glass-panel border-zinc-800 overflow-hidden flex items-center justify-center bg-zinc-900">
            {loadingImage ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-xs uppercase tracking-widest animate-pulse text-rose-500">
                  Conjuring Moodboard...
                </span>
              </div>
            ) : imageUrl ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 text-center text-zinc-600 gap-4">
                <span className="block text-4xl opacity-20">☾</span>
                <p className="text-xs uppercase tracking-widest">
                  Visual Manifestation Ready
                </p>
                <button
                  onClick={handleGenerateImage}
                  className="px-6 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs uppercase tracking-widest transition-colors border border-zinc-700 hover:border-zinc-500"
                >
                  Generate Aura
                </button>
              </div>
          ...
          {result.cosmic && (
            <div className="glass-panel p-6 bg-gradient-to-br from-purple-900/20 to-black border border-purple-500/20 relative overflow-hidden">
              ...
              <h4 className="text-xl font-display text-zinc-200 mb-2">
                {result.cosmic.zodiacHeader}
              </h4>
              <p className="text-sm text-zinc-400 leading-relaxed">
                {result.cosmic.analysis}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-8">
          <div className="text-center lg:text-left">
            ...
            <p className="text-xl text-rose-500/80 font-display">
              {result.subtitle}
            </p>
          </div>
            ...
            <>
              ...
              <div className="space-y-2">
                <h3 className="text-xs font-bold uppercase tracking-widest text-blue-400">
                  2026 Seduction Prophecy
                </h3>
                <ul className="space-y-3">
                  {result.cosmic.prophecy.map((item, idx) => (
                    <li
                      key={idx}
                      className="flex gap-3 text-sm text-zinc-300 bg-zinc-900/50 p-3 rounded border border-zinc-800"
                    >
      ...
      <div className="mt-16 pb-8 text-center">
        <p className="text-zinc-700 text-[10px] uppercase tracking-widest mb-4">
          AuraType Analysis Engine v2.5
        </p>
        <div className="inline-flex flex-col items-center gap-2 p-4 border border-zinc-900 bg-zinc-950/50 rounded-sm">
          ...
          <a
            ...
            className="text-xs font-bold text-zinc-400 hover:text-rose-500 transition-colors uppercase tracking-widest"
          >
            @RafayYousafzai
          </a>
        </div>
